...
function savePreviewAsPhoto() {
  ...
  const lang = localStorage.getItem('calendar_lang') || 'zh';
  window.location.href = `/poster/${activePreviewEvent.id}?lang=${lang}&format=desktop`;
}
...
async function uploadImageFile(file) {
  ...
  try {
    ...
    if (eventBgOpacity.value == 100) {
      eventBgOpacity.value = 40;
